...
## Aşama 2: Veri Çekme ve Analiz Modülü (Python) - [Tamamlandı]
...
## Aşama 3: AI Model Entegrasyonu (Ollama) - [Tamamlandı]
- [x] Local Ollama kurulumu (örn. Llama 3 veya finansal modeller).
- [x] Python üzerinden Ollama API'sine bağlanılması.
- [x] Hisse verilerini analiz etmek üzere uygun Prompt Engineering yapılması.
- **AI Katkısı (Skills Agent):** LLM'e gidecek promptların tasarlanması.

## Aşama 4: Performans Optimizasyonu (C++) - [Tamamlandı]
- [x] Python'da yavaş çalışan tarama döngülerinin C++'a taşınması (pybind11 veya gRPC).
- [x] C++ kodunun derlenip Python içerisinden çağrılması.

## Aşama 5: Kullanıcı Arayüzü (C#) - [Tamamlandı]
- [x] C# ile masaüstü (veya Web) arayüzü oluşturulması.
- [x] Python API'si ile C# arayüzü arasında HTTP/WebSocket bağlantısı kurulması.
- [x] Kullanıcının hisse kriterlerini girmesi ve analiz sonuçlarını/AI yorumlarını ekranda görmesi.

## Aşama 6: Docker ve Dağıtım (Deployment) - [Bekliyor]
- [ ] **Taşınabilir EXE Oluşturma**: Tüm sistemin (Python API + C# UI) tek bir EXE dosyasında toplanması.
